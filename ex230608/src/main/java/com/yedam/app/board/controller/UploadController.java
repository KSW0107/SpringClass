...
public class UploadController {
	...
	@ResponseBody
	public void uploadFile(@RequestPart MultipartFile[] uploadFiles) { // @RequestPart 첨부파일 사용 시 사용 (주로 배열 사용)

		for (MultipartFile uploadFile : uploadFiles) {

			if (uploadFile.getContentType().startsWith("image") == false) { // 첨부파일을 이미지만 처리하겠다는 구문 
	...
	private String makeFolder() {
		...
		// LocalDate를 문자열로 포멧
		String folderPath = str.replace("/", File.separator); // File.separator = 자바가 인식하는 경로 구분 형식 (/ -> File.separator)
